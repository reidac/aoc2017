...
-- Part 2, doesnt return the map, just tracks the largest value seen so far.
runpgm :: Map.Map String Int -> [(Operation, Condition)] -> Int -> Int
runpgm m [] v = v
runpgm m p v = do
  let (op,cn) = head p
  let nm = case (ceval cn m) of True -> (perfop op m)
                                False -> m
  let localmax = maximum (Map.elems nm)
  -- For some reason, the "maximum" operator doesn't work here .. ?
  let globalmax = case (localmax > v) of True -> localmax
                                         False -> v
  runpgm nm (tail p) globalmax
