...
trverse (cr,cc) (fr,fc) m s = do
  ...
  if (cchar == ' ') then s else do -- Terminate when you run off the end.
    ...
    let news = if isLetter cchar then (s ++ [cchar]) else s
    if (cchar /= '+') then trverse (cr+fr, cc+fc) (fr,fc) m news else do
      ...
      if (lchar /= ' ') then trverse (cr+lr, cc+lc) (lr, lc) m news
                        else trverse (cr+rr, cc+rc) (rr, rc) m news
